...
CString CCalendar::GetLunarYear()
{
	...
	{
		WORD iLunarYear,iLunarMonth,iLunarDay;  //农历年月日
...
CString CCalendar::GetHoliday()
{
	...
	{
		CString sMsg;
		sMsg.Format(L"目录<%s>不存在！",sHolidayPath);
		AfxMessageBox(sMsg);
		return L"";
	}
...
CString CCalendar::GetHolidayForDate(int iAddDay,vector<HolidayInfo> &vecInfo)
{
	...
	stmp.Empty();
	stmp=GetGetLunarHolDay(iYear,iMonth,iDay);  //获取农历节气
	if(stmp!=L"")
	{
		sHoliday.Append(stmp);
		sHoliday.Append(L"\n");
...
{
	CTime tmToday(m_iYear,m_iMonth,m_iDay,0,0,0); //今天的日期 
	CTime tmNewDay; //前两天的日期 
	CTimeSpan tmspanTwoDay(iAddDay,0,0,0); //两天的时间距离 
